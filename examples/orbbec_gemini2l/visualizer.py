...
class RerunVisualizer:
    ...
    def visualize_frame(
        ...
        # Log the latest trajectory at the current time step
        # Using static=True to ensure it persists and updates in the viewer
        # Wrap trajectory in a list to treat it as a single strip
        if len(trajectory) > 1:
            traj_array = np.array(trajectory)
            rr.log(
                "world/trajectory", 
                rr.LineStrips3D([traj_array], colors=[[255, 255, 255]], radii=0.02),
                static=True
            )
